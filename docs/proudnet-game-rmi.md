...
(See [proudnet-offline-protocol.md](proudnet-offline-protocol.md) for the full handshake. Repeated here only as it pertains to RMI.)
...
| **ProudNet MessageType** (ordinal) | `1`=Rmi, `2`=UserMessage, `37/38`=Compressed, `39`=Encrypted | first byte read by `Message_Read sub_D59300` inside `sub_D653B0`; see [proudnet-message-dispatch-map.md](proudnet-message-dispatch-map.md) |
...
|--------|-----------|-----------|-----|---------------|---------------|
| connect (NetUserConnect) | — | — | — | `0x3F3E` → `sub_4BA070` | 0x180 body; stores account/session (see proud_rmi.py) |
...
## 10. In-process injection technique (our harness) **[V]**
...
- **Pump** on the drain hook (`sub_D65940` → `drain_receive_queue`): consume the flag and call the leaf. This runs on the **receive/dispatch thread** — the same thread the real dispatcher would use, so the leaf's `RequestState`/UI calls happen in the right context.
...
## 11. Open questions / suspicions / corrections
