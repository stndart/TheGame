...
| `0x14314E0` | `w_connect_2` | Alternate connect helper (~591 B); connect preamble patched @ **`0x14316A2`**. | `w_connect_2.cpp` (**trampoline**) | — |
...
| `0xD71CE0` | `sub_D71CE0` | `pn_recv_append.cpp` | **full jmp** | `PNRecvBuffer::append` — S2C hex via `SocketTrace`; growth still via direct call to `sub_D71610` (`pn_recv_append.cpp`). |
...
| `0x014316A2` | `w_connect_2` (patch) | `w_connect_2.cpp` | **trampoline** | Remap sockaddr at connect preamble in `sub_14314E0`, then `connect_syshandle`, then resume original @ `return_addr`. Parent RVA `0x14314E0`. |

**Related (same path, different mode):**

| RVA | Hook | Mode | Notes |
|-----|------|------|-------|
| `0xD567F0` | `send.cpp` `hook_send_2` | **trace** | PN worker TX hex; falls through to original. |
...
| Required | `w_connect_3` + `w_connect_2` + `connect` IAT | `w_connect_3` full jmp; `w_connect_2` trampoline for alternate preamble — not redundant with vtable slot |
...
| Useful | `w_wsasend_1` (`send_2`) | **trace** — PN worker TX hex if not all traffic uses `TCPSocket::Send` |
...
*Last updated: 2026-05-28 — hook index: `pn_upnp`, `pn_select`, `pn_recv_append`, `w_connect_3` (**full jmp**); `w_connect_2` (**trampoline** @ `0x14316A2`). Matches `src/main.cpp`.*
