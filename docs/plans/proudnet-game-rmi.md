...
**Verify:** `cd server && uv run python -m server.test_wire_create_room` and `uv run python -m server.test_wire_start_match`; live: Create Room → Ready, check TX for `023d3f` and `game_stage` → map load. Use default **debug** DLL (inject compiled out) to prove wire-only.
...
- **Pump** on **main thread** from `game_stage` hooks (`room_list` / `room` `onPreProcess` prologues in [`game_stage.cpp`](../src/hooks/game_stage.cpp)) - not from `pn_drain_recv` (worker-thread pump raced the UI VM). Default **debug** build has inject off; reconfigure `-DTHEGAME_DISABLE_RMI_INJECT=OFF` to test in-process RES.

**Verified result (run 180):** create-room REQ `0x3F30` → `inject: create-room RES 0x3F30 result=0 -> GameRoom` → `game_stage: room`. Client entered `CGameRoom`. **[V]**
