...
unsigned long remap_ipv4(unsigned long ipv4) {
  ...
  if (override_ip.empty())
    return 0;
  const unsigned long entry = inet_addr(kEntryHost);
  const unsigned long lobby = inet_addr(kLobbyHost);
  if (ipv4 != entry && ipv4 != lobby)
    return 0;
  return inet_addr(override_ip.c_str());
}
