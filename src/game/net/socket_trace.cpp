...
bool is_game_server_socket(SOCKET s) {
  return is_entry_socket(s);
}
...
SOCKET read_fast_socket_slot(void *fast_socket_ctx) {
  ...
  auto *sock_slot = reinterpret_cast<SOCKET *>(static_cast<char *>(fast_socket_ctx) +
                                               300);
...
void sync_fast_socket_handle(void *fast_socket_ctx) {
  ...
  // GAME / PROBE legs must keep their own fd — overwriting caused 0x2f on :7000.
