...
#include "game/net/pn_select_fd.hpp"
...
void set_fast_socket_nonblock(PNFastSocket *fast) {
  if (!fast)
    return;
  auto *base = reinterpret_cast<char *>(fast);
  SOCKET s = *reinterpret_cast<SOCKET *>(base + pn::fast_socket::kSocket);
  u_long mode = 1;
...
// gateway.
char retry_connect_on_poll_except(PNConnectionNode *conn) {
  auto *base = reinterpret_cast<char *>(conn);
  auto *gateway = *reinterpret_cast<char **>(base + pn::conn::kGatewayCtx);
  if (!gateway) {
    conn->fsm_state = 5;
    return 0;
  }
  ...
  PNFastSocket *const sock = conn->fast();
  if (!sock) {
    conn->fsm_state = 5;
    return 0;
  }
...
PNSendArm &PNConnectionNode::send_arm() {
  return *reinterpret_cast<PNSendArm *>(reinterpret_cast<char *>(this) +
                                        pn::conn::kSendArm);
}

PNFastSocket *PNConnectionNode::fast() const {
  if (!fast_socket)
    return nullptr;
  return *fast_socket;
}

int PNConnectionNode::outbound_remaining() const {
  const auto *self = this;
  const auto &arm = const_cast<PNConnectionNode *>(self)->send_arm();
  const int total = static_cast<int>(arm.outbound.GetLength());
  return total - arm.send_offset;
}

bool PNSendArm::arm_send() {
...
char PNConnectionNode::run_state_1() {
  logf("pn_conn: state1 conn=%p fast=%p", this, fast());

  alignas(4) std::uint8_t select_buf[0x220];
  auto *sel = reinterpret_cast<PNSelectContext *>(select_buf);
  pn::select_fd::init(sel);

  PNFastSocket *const sock = fast();
  if (sock) {
    pn::select_fd::add_read(sel, sock);
    pn::select_fd::add_except(sel, sock);
  }

  pn::select_fd::wait(sel, 0);
  ...
  const int upnp_mode = *reinterpret_cast<int *>(base + pn::conn::kUpnpSubmode);
  auto *upnp = reinterpret_cast<PNUpnpClient *>(this);
  if (upnp_mode == 1) {
...
char PNConnectionNode::run_state_2() {
  auto *base = reinterpret_cast<char *>(this);
  ...
  base[pn::conn::kSendPendingConn] = 0;
  ...
  *reinterpret_cast<int *>(base + pn::conn::kSendBytesDone) += bytes;
  ...
  base[pn::conn::kRecvPending] = 1;
  if (sock->recv(0x8000) == 0) {
    fsm_state = 3;
    return 0;
  }

  base[pn::conn::kRecvPending] = 0;
  fsm_state = 5;
  return 0;
}

int PNConnectionNode::run_state_3() {
  auto *base = reinterpret_cast<char *>(this);
  std::uint32_t overlap_out[8] = {};
  ...
  *reinterpret_cast<int *>(base + pn::conn::kRecvBytesTotal) += chunk;

  const int remaining =
      0x8000 - *reinterpret_cast<int *>(base + pn::conn::kRecvBytesTotal);
  if (remaining <= 0) {
    fsm_state = 4;
    return 0;
  }
